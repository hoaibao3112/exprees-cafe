...
export default function AdminEditArticlePage() {
  ...
  const { data: article, isLoading, error } = useQuery({
    ...
    queryFn: () => adminArticlesApi.getById(id) as Promise<Article & { contentHtml?: string }>,
    enabled: !!id,
  });
  ...
  const updateMutation = useMutation({
    mutationFn: (values: ArticleFormValues) => {
      ...
      return adminArticlesApi.update(id, payload);
    },
    onSuccess: () => {
    ...
    onError: (err: Error) => {
      toast.error(err.message || 'Có lỗi xảy ra khi cập nhật bài viết');
    },
  });
