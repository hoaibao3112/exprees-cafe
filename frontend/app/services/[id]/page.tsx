...
export default function ServiceDetailPage(props: { params: Promise<{ id: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#f8f7f4] text-zinc-800 font-sans">
      ...
      ══════════════════════════════════════ */}
      <div className="relative w-full h-[48vh] min-h-[320px] overflow-hidden">
        {/* Background image */}
        <OptimizedImage
        ...
        <div className="absolute bottom-0 left-0 right-0 max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8">
          <span className="inline-block px-3 py-1 bg-orange-500 text-white text-[10px] font-extrabold uppercase tracking-widest rounded-md mb-3">
            Dịch vụ F&B
          </span>
        ...
        {serviceImages.length > 1 && (
          <>
            <button
              onClick={handlePrev}
              className="absolute left-4 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/20 backdrop-blur-sm hover:bg-white/40 rounded-full flex items-center justify-center transition-all text-white"
            >
              <ChevronLeft className="w-5 h-5" />
            </button>
            <button
              onClick={handleNext}
              className="absolute right-4 top-1/2 -translate-y-1/2 w-9 h-9 bg-white/20 backdrop-blur-sm hover:bg-white/40 rounded-full flex items-center justify-center transition-all text-white"
            >
              <ChevronRight className="w-5 h-5" />
            </button>
            <div className="absolute top-5 right-4 bg-black/40 backdrop-blur-sm text-white text-[10px] font-bold px-2.5 py-1 rounded-full">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <div className="grid grid-cols-3 gap-3">
              {[
                ...
                <div
                  key={text}
                  className="bg-white rounded-xl border border-zinc-100 p-4 flex flex-col items-start gap-2 shadow-sm"
                >
